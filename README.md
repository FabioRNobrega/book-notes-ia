...
- Optional .NET 10 EPUB 2/3 chapter-text parser proof of concept (isolated container)
- Docker Compose
...
The EPUB parser is a separate POC and is not part of the normal application stack. Place a legally obtained supported EPUB file under `services/EbookParseService.Api/data/input/`, then run:

```bash
make ebook-parse BOOK=book.epub
```

If an EPUB was repackaged with a misplaced media-type marker, repair its container first:

```bash
make repair-ebook BOOK=book.epub
```

This preserves `book.epub` and writes `book-fixed.epub` beside it. It repairs only ZIP/EPUB marker packaging; it does not convert EPUB 2 NCX navigation into EPUB 3 navigation or repair chapter structure.

It follows EPUB container, package, and navigation metadata and writes one navigation-derived chapter per file under the ignored `data/output/<book-slug>/` folder. It supports EPUB 3 navigation fragments and a constrained EPUB 2 NCX fallback for `Chapter N`, plain `N`, or numbered-title (`N. Title`) entries whose XHTML target begins with the same numeric heading; a whole-document target includes its following spine resources up to the next qualifying chapter target. When an NCX package omits `dc:language`, every accepted chapter must provide the same explicit `xml:lang`. Reruns stage a complete set before replacing the prior output. Ambiguous structures and encrypted content are rejected rather than guessed. The source book and extracted text stay local and must not be committed. This POC does not call Chatterbox, Supertonic, WebApp, or a database. See [the parser service README](services/EbookParseService.Api/README.md) for supported EPUB structures, limits, tests, and privacy details.
...
├── services/
│   ├── EbookParseService.Api/   # Isolated EPUB 2/3 chapter-text parser POC
...
The POC defaults to Chatterbox Multilingual V3 on CPU with separate English (`en`) and Portuguese (`pt`) reference slots and supports multiple preserved voices per language. An opt-in English-only Nano benchmark can reuse an existing voice ID without replacing its Multilingual artifacts. Its source package reports version `0.1.7` and is pinned to official Chatterbox commit `5de7a54aa4e5e2baadb0182dde554908b48b85c2`, because the PyPI wheel with the same version does not yet contain the required V3/Nano loading APIs. The complete Linux/Python 3.11 dependency graph is recorded in `requirements.lock.txt`, the image explicitly installs PyTorch's CPU-only wheels, and model files use immutable Hugging Face revisions. The model cache, archived references, generated UUID voice metadata, conditioning `.pt` files, and generated audio remain local and ignored by Git.
...
Nano stores `conditioning-nano.pt`, its integrity sidecar, and `preview-en-nano.wav` beside—but independently from—the same voice's Multilingual artifacts. The response reports `real_time_factor` for CPU benchmark comparison.
