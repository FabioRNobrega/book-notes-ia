...
The `webapp` container runs the full `.NET SDK 10.0` image (not a slim runtime). When the stack is running you can open a shell inside it and use any `dotnet` command:
...
> **Note:** `make test` spins up a separate, ephemeral `mcr.microsoft.com/dotnet/sdk:10.0` container via `docker-compose.test.yml`. Use it for clean CI-style test runs. Use `docker compose exec webapp bash` for interactive development against the already-running stack.
