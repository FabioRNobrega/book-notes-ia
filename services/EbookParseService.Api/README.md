...
For EPUB 2 NCX, an entry is a chapter only when its label is `Chapter N`, plain `N`, or a numbered title such as `N. Title`; its target must be a manifest-declared XHTML document containing a heading that begins with the same numeric chapter number (for example, `N TITLE`). Whole-document, fragment, and nested NCX targets are supported. A whole-document target includes following XHTML spine resources up to the next qualifying whole-document NCX target, so title-only target files retain their chapter prose. Non-chapter NCX entries are ignored. If the package omits `dc:language`, every accepted chapter must have the same explicit in-scope `xml:lang`; language is never guessed from filenames or prose. XHTML `&nbsp;` is normalized locally without resolving an external DTD.
